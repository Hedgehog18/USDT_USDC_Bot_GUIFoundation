        ...
        with self.connect() as conn:
            row = conn.execute(
                ...
                SELECT
                    ...
                    COALESCE(SUM(CASE WHEN status = 'CLOSED' AND (close_reason = 'target' OR close_reason LIKE '%target%') THEN 1 ELSE 0 END), 0) AS target_closed,
                    COALESCE(SUM(CASE WHEN status = 'CLOSED' AND (close_reason = 'target' OR close_reason LIKE '%target%') THEN net_profit ELSE 0 END), 0) AS target_total_profit,
                    ...
                    COALESCE(SUM(CASE WHEN status IN ('CLOSED', 'CLOSED_MANUAL') AND was_near_target = 1 AND NOT (close_reason = 'target' OR close_reason LIKE '%target%') THEN 1 ELSE 0 END), 0) AS missed_target_count,
