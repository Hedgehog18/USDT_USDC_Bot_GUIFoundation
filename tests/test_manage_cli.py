...
def test_manage_cli_hf_regime_filter_sim_accepts_since_id_limit_and_threshold():
    ...
    assert args.command == "hf-regime-filter-sim"
    assert args.profile == "mean_reversion_hf_micro_v1"
    assert args.since_id == 1534
    assert args.limit == 100
...
def test_manage_cli_extreme_replay_accepts_profile_and_output():
    parser = build_parser()
    args = parser.parse_args([
        "extreme-replay",
        "--profile",
        "mean_reversion_hf_micro_v1",
        "--output",
        "reports/custom_extreme_replay.txt",
    ])
...
def test_manage_cli_extreme_signal_discovery_accepts_profile_and_output():
    parser = build_parser()
    args = parser.parse_args([
        ...
        "mean_reversion_hf_micro_v1",
        "--output",
        "reports/custom_extreme_signal_discovery.txt",
    ])
